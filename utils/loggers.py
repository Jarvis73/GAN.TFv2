...
def get_global_logger(model_dir=None, mode=None, log_file=None, name="default"):
    global logger
    if logger.default:
        if model_dir is None:
            ...
            if log_file:
                log_file_full = log_dir / log_file
                with_time = False
            else:
                log_file_full = log_dir / "{}".format(mode or "mode")
                with_time = True
            logger._log = create_logger(log_file=log_file_full, file_=True, with_time=with_time, file_level=1,
                                        clear_exist_handlers=True, name=name)
